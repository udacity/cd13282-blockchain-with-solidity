...
contract EventTicketing {
    ...
    uint public endTime;
    address public owner; // State variable to represent the event organizer.

    // Modifier to restrict actions to the event organizer.
    modifier onlyOwner() {
        require(msg.sender == owner, "Only the event organizer can perform this action.");
        _;
    }
    ...
    constructor(uint _startTime, uint _endTime) {
        ...
        endTime = _endTime;
        owner = msg.sender; // Assign the contract deployer as the event organizer.
    }
